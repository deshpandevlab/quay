...
class Sentry(object):
            ...
            if sentry_dsn:
                try:
                    ...
                    initialized_sentry = sentry_sdk.init(
                        ...
                        # Fix connection pool issues with Sentry
                    ...
                    # Send a test message to verify connectivity (only in non-production)
                    if app.config.get("SENTRY_ENVIRONMENT") != "production":
                        try:
                            sentry_sdk.capture_message(
                                "Sentry initialization test message", level="info"
                            )
                            logger.info("Sentry test message sent successfully")
                        except Exception as test_e:
                            logger.warning("Failed to send Sentry test message: %s", str(test_e))
    ...
    def test_sentry_connection(self):
        ...
        if self.state and not isinstance(self.state, FakeSentry):
            try:
